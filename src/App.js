  ...
  return (
    <div className="flex h-screen bg-[#020202] text-white font-sans overflow-hidden transition-colors duration-1000">
      ...
      <main className="flex-1 flex flex-col relative z-10 overflow-hidden">
        ...
        {/* Header */}
        <header className="px-8 lg:px-12 pt-6 lg:pt-8 flex items-end justify-between shrink-0">
          <div className="flex items-center gap-6 animate-slide-in">
            ...
            <div>
              <h1 className="text-3xl lg:text-5xl font-black tracking-tighter leading-none mb-2">
                {currentPlaylistStyle.label}
              </h1>
              <div className={`h-1 w-12 lg:w-16 bg-gradient-to-r ${currentPlaylistStyle.color} rounded-full shadow-lg shadow-white/5`} />
            </div>
          </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none -z-10">
          <span className="text-[25vw] font-black opacity-[0.03] uppercase tracking-tighter transition-all duration-1000">
        ...
        {/* Player Controls Bar */}
        <footer className="h-24 lg:h-28 px-6 lg:px-10 flex items-center justify-between bg-black/40 backdrop-blur-3xl mx-4 lg:mx-8 mb-6 lg:mb-8 rounded-[1.5rem] lg:rounded-[2.5rem] border border-white/10 shadow-2xl shrink-0">
          <div className="w-1/3 flex items-center gap-4">
             {currentTrack && (
               <>
                 <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl overflow-hidden hidden sm:block ring-1 ring-white/10">
                    <img src={currentTrack.cover} alt="" className="w-full h-full object-cover" />
                 </div>
                 <div className="flex flex-col min-w-0">
                    <span className="text-[8px] font-black uppercase tracking-[0.2em] text-neutral-500 mb-0.5">NOW PLAYING</span>
                    <span className="text-sm lg:text-lg font-black truncate text-white leading-none">{currentTrack.title}</span>
                 </div>
               </>
             )}
          </div>

          <div className="flex flex-col items-center gap-2 lg:gap-3 w-1/3">
             <div className="flex items-center gap-6 lg:gap-8">
                <button onClick={handlePrev} className="text-neutral-500 hover:text-white active:scale-90"><SkipBack size={18} fill="currentColor" /></button>
                <button 
                  onClick={() => setIsPlaying(!isPlaying)} 
                  className={`w-10 h-10 lg:w-12 lg:h-12 bg-white text-black rounded-full flex items-center justify-center transition-transform hover:scale-110 active:scale-90`}
                >
                  {isPlaying ? <Pause size={20} fill="black" /> : <Play size={20} fill="black" className="ml-0.5" />}
                </button>
                <button onClick={handleNext} className="text-neutral-500 hover:text-white active:scale-90"><SkipForward size={18} fill="currentColor" /></button>
             </div>
